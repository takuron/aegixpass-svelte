...
AegixPass V2 具有以下安全目标与特性：

- **确定性**：相同输入始终生成相同密码，无需保存派生结果。
- **抗离线猜测成本**：Argon2id / Scrypt 通过内存和时间成本极大增加了对主密码的暴力破解难度。
- **参数绑定**：`platformId`、`distinguish_key`、`length`、`charsets` 及算法配置都会影响最终哈希计算，起到全方位雪崩效应。
- **位置不可预测**：保证字符经过最终洗牌后不再固定出现在密码前部。
- **无偏抽样**：字符选择和洗牌索引强制使用拒绝采样，确保字符被选中概率公允。
- **注意事项**：快哈希预处理是为了结构化输入数据，不会增加弱主密码本身的熵，安全性仍主要依赖主密码自身的复杂度以及慢哈希带来的计算成本。`platformId` 通常被公开或容易获取，它仅作为域分离的盐值/偏移值，绝不能替代主密码的安全职责。
