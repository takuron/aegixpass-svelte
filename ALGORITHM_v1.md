...
1. **拼接输入数据**：将所有影响密码生成的参数和用户输入拼接成一个唯一的、格式固定的长字符串。
    ...
    - 示例:
      ...
      ```
    - 这种设计确保了预设中的任何一个参数（甚至是 `charsets` 的顺序）发生变化，都会生成一个完全不同的种子。注意：`charsets_json` 必须由标准 JSON 序列化得到。
2. **哈希计算**：根据 hashAlgorithm 的类型，使用不同的方法处理上一步的输入数据：
    - **如果使用快哈希** (sha256, blake3, sha3_256): 直接对上一步拼接好的 UTF-8 编码字符串进行哈希计算，得到 32 字节的主种子。
    - **如果使用慢哈希** (argon2id, scrypt): 慢哈希是内存困难型函数，需要额外的盐（Salt）/偏移量（Offset）和计算参数来增加破解难度。
      - a. **生成确定性盐值（初始偏移）**：为了保证整个流程的确定性，慢哈希所需的盐值由 platformId 通过 SHA-256 哈希生成：`salt = sha256(platformId)`。这个生成的 32 字节哈希结果被直接用作慢哈希算法的盐值或初始偏移量。
      - b. **执行密钥派生**：
        - 对于 argon2id：使用 Argon2id 算法，结合预设的参数（内存成本: 19 MiB, 迭代次数: 2, 并行度: 1），处理输入数据和确定的盐值/偏移量，派生出 32 字节的主种子。
        - 对于 scrypt：使用 Scrypt 算法，结合预设的参数（N=2^15, r=8, p=1），处理输入数据和确定的盐值/偏移量，派生出 32 字节的主种子。
...
2. **合并字符集**：将 `charsets` 数组中的所有字符按原顺序合并成一个大的字符池。

3. **填充字符**：循环填充剩余长度的每一个位置：
    - 从 RNG 中获取一个随机数。
    - 为了保证公平性，这里使用了一个无偏的范围随机数生成逻辑（拒绝采样），它通过“拒绝采样”方法避免了简单取模运算带来的偏差，确保大字符池中的每个字符被选中的概率完全相等。
    - 将从字符池中选出的字符添加到密码数组中。
...
2. **Fisher-Yates 洗牌**：从后向前遍历密码数组，对于每个位置 `i`，使用无偏的范围随机数生成逻辑（拒绝采样）生成一个 `[0, i]` 范围内的随机索引 `j`，然后交换位置 `i` 和 `j` 的字符。
...
- **无偏范围随机数**：生成范围随机数时必须使用拒绝采样，避免简单大数取模造成的概率不公。
- 文本处理应严格使用 UTF-8 编码，并且对字符的处理建议基于 Unicode 标量值。
